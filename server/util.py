...
def classify_image(image_base64_data, file_path = None):
    ...
    for img in imgs:
        # resize image and create a wavelet transfrom and then vertically stack the raw image ontop of the wavelet image
        ...
        final_image = combined_img.reshape(1, len_image_array).astype(float)

        # predict on the image supplied using the ML model built
        result.append({
...
def get_cv2_image_from_base64_string(b64str):
    ...
    takes a base64 string and returns a cv2 image
    '''
...
def get_cropped_image_if_2_eyes(image_path, image_base64_data):
    # similar function as feature extraction function of faces with 2 eyes.
